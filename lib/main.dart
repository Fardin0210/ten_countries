...
class _MyApplicationState extends State<SplashScreen> {
  ...
  Widget build(BuildContext context) {
    return AnimatedSplashScreen(
      splash: Center(
        child: Lottie.asset('animations/Animation - 1731497253668.json'),
      ),
      nextScreen: homePage(),
      duration: 3500,
      ...
      body: Padding(
        ...
        child: ListView(
          children: <Widget>[
            ...
            // فرم ارسال پیام (با استفاده از TextField برای ورودی کاربر)
            Text(
              'ارسال پیام:',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
            ),
            ...
            ElevatedButton(
              onPressed: () {
                // عملکرد ارسال پیام (موقتاً فقط یک پیغام در SnackBar)
                ScaffoldMessenger.of(context).showSnackBar(
...
class CountryDetailPage extends StatelessWidget {
  ...
  void launchURL() async {
    ...
    // بررسی امکان باز کردن URL
    if (await canLaunchUrl(Uri.parse(url))) {
      // باز کردن URL
      await launchUrl(Uri.parse(url));
    } else {
      // اگر نتوانست URL را باز کند، خطا می‌دهیم
      throw 'Could not launch $url';
    }
  }
